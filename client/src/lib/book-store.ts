...
export async function getBooks(): Promise<BookMeta[]> {
  try {
    ...
    if (!response.ok) {
      bookLogger.error("Failed to fetch books from server");
    ...
    bookLogger.error("Error fetching books:", error);
...
export async function getBook(id: string): Promise<BookMeta | null> {
  ...
  try {
    ...
    if (!response.ok) {
      bookLogger.error("Failed to fetch book from server");
    ...
    bookLogger.error("Error fetching book:", error);
...
export async function saveBook(book: BookMeta): Promise<BookMeta> {
  ...
  if (book.pdfData) {
    try {
      ...
      bookLogger.error('Error uploading book:', error);
      throw error;
    }
  } else {
...
export async function deleteBook(id: string): Promise<BookMeta[]> {
  ...
  try {
    ...
    bookLogger.error('Error deleting book:', error);
    throw error;
  }
}
